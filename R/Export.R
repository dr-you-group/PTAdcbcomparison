...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "PTAdcbcomparison3")
  tcosOfInterest <- read.csv(pathToCsv, stringsAsFactors = FALSE)
  pathToCsv <- system.file("settings", "CohortsToCreate.csv", package = "PTAdcbcomparison3")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(name, ".json"), package = "PTAdcbcomparison3")
...
exportOutcomes <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "NegativeControls.csv", package = "PTAdcbcomparison3")
...
exportDiagnostics <- function(outputFolder,
  ...
  cluster <- ParallelLogger::makeCluster(min(4, maxCores))
  ParallelLogger::clusterRequire(cluster, "PTAdcbcomparison3")
